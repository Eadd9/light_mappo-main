...
class DiscreteActionEnv(object):
    ...
    def __init__(self):
        ...
        # configure spaces
        # 和单智能体比较大的区别就是有这个obs space（观测）
        self.action_space = []
        self.observation_space = []
        self.share_observation_space = [] # 共享obs，作为critic的输入

        share_obs_dim = 0
        total_action_space = []
        for agent in range(self.num_agent): #遍历智能体
            ...
            # total action space
            if len(total_action_space) > 1:  # 动作空间大于1时要做更多处理
